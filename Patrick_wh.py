...
characters = {
    "PATRICK2", "NICHOLAS2", "ANNETTE2", "JULIA2", "DAVID2", "JOHNNY2", "ANNE2", "WILLY2", "ELEANOR2", "MARIANNE2", "BRIDGET2", "SONNY2", "VIRGINIA2", "MARY2", "ROBERT2", "SEAMUS2", "NANCY2"
}
...
def build_relationship_map(scene, speakers, mentioned, num_present_speakers):
    for speaker, speaker_weight in speakers.items():
        ...
        for mention, mention_weight in mentioned.items():
            ...
            count = len(matches)
            # print(f"Found {count} occurrences of '{mention}' in scene for speaker '{speaker}', including weight {num_present_speakers}.")
            weighted_count = (count * (speaker_weight + mention_weight)) / num_present_speakers if num_present_speakers > 0 else 0
...
text_files = [
    ...
    # Add more filenames here, e.g. "AnotherScript.txt"
]
...
excel_filename = f"norm_relationship_map_w1_{w1:.1f}_w2_{w2:.1f}.xlsx"
